...
@section('title', 'Kitchen Ingredients — All The Season Garden')
    ...
    $(document).ready(function() {
        ...
        document.querySelectorAll('.edit-ingredient-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                var actionUrl = "{{ route('admin.kitchen.ingredients.update', ':id') }}".replace(':id', id);
                document.getElementById('editIngredientForm').setAttribute('action', actionUrl);
        ...
        document.querySelectorAll('.delete-ingredient-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                var id = this.getAttribute('data-id');
                var actionUrl = "{{ route('admin.kitchen.ingredients.destroy', ':id') }}".replace(':id', id);
                document.getElementById('deleteIngredientForm').setAttribute('action', actionUrl);
            });
        });
    });
</script>
...
@section('content')
<div class="content-wrapper ingr-wrap">
    
    @include('partials.message-bag')

    {{-- Page Header --}}
    <div class="ingr-header">
        <div class="ingr-title-group">
            <h1>Raw Materials & Ingredients</h1>
            <p>Manage kitchen ingredients, units of measure, and stock threshold alerts.</p>
        </div>
        <button class="btn-add-ingr" data-bs-toggle="modal" data-bs-target="#addIngredientModal">
    ...
    {{-- Ingredients Card --}}
    <div class="ingr-card">
        <div class="ingr-card-header">
            <h3 class="ingr-card-title">Inventory List ({{ $ingredients->count() }})</h3>
        </div>
            ...
            <div class="table-responsive">
                <table class="table" id="ingredients-table">
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($item->quantity <= $item->alert_quantity)
                                        <span class="badge bg-warning text-dark fw-semibold" style="font-size: 11px;">Low Stock</span>
                                    @else
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-1">
                                        <button class="btn btn-sm btn-outline-secondary edit-ingredient-btn"
                                            data-id="{{ $item->id }}"
                                            data-name="{{ $item->name }}"
                                            data-unit="{{ $item->unit }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">No raw material ingredients added yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal fade" id="addIngredientModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form action="{{ route('admin.kitchen.ingredients.store') }}" method="POST" style="width: 100%;">
                @csrf
                <div class="modal-content border-0" style="border-radius: 10px;">
                    <div class="modal-header border-0 pb-0">
                        <h5 class="modal-title font-weight-bold">Add Raw Material</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body py-3">
                        <div class="row g-2">
                            <div class="col-md-6 mb-2">
                                <label class="fw-semibold mb-1" style="font-size: 12px;">Ingredient Name *</label>
                                <input type="text" name="name" class="form-control" required placeholder="e.g. Onions, Cooking Oil, Rice" style="font-size: 13px;">
                            </div>
                            <div class="col-md-6 mb-2">
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <textarea name="description" class="form-control" rows="2" placeholder="e.g. Fresh red onions for burgers and salads" style="font-size: 13px;"></textarea>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="editIngredientModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form id="editIngredientForm" method="POST" style="width: 100%;">
                @csrf
                @method('PUT')
                <div class="modal-content border-0" style="border-radius: 10px;">
                    <div class="modal-header border-0 pb-0">
                        <h5 class="modal-title font-weight-bold">Edit Raw Material</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body py-3">
                        <div class="row g-2">
                    ...
                    <div class="modal-footer border-0 pt-0 pb-3">
                        <button type="button" class="btn btn-light px-4 me-2" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-danger px-4 font-weight-bold">Update Ingredient</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="modal fade" id="deleteIngredientModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form id="deleteIngredientForm" method="POST" style="width: 100%;">
                @csrf
                @method('DELETE')
                <div class="modal-content border-0" style="border-radius: 10px;">
                    <div class="modal-header border-0 pb-0">
                    ...
                    <div class="modal-body text-center py-4" style="font-size: 13.5px; color: #4b5563;">
                        Are you sure you want to delete this raw material ingredient?
                    </div>
